...
#Path is datafile, SEC_HZ is acquisition frequency (e.g. 40 frames per second)
PATH = r''                              # insert recorded data file name
SEC_HZ = 40                             # acquisition frequency
BASE_LENGTH = 300                       # baseline time period (s) before intervention
CUT = 10                                # eliminate start of rec for bleaching / artefacts
REC_LENGTH = 35 * 60                    # full recording length (min * sec)
FRAME_LENGTH = REC_LENGTH * SEC_HZ      # full frame length
SMALL_SIZE = 12
MEDIUM_SIZE = 20
BIGGER_SIZE = 36
...
# read in doric files for photometry data. both for single and dual recordings
...
# correct for different emission output of isosbestic signal. use only the baseline prior to intervention where there should not be any measurable signal in the 465
def scaling_factor(analog1, analog2):
    ...
    an1_length = len(analog1)
    an2 = analog2[:an1_length]
    array = analog1 - (an2 * correction_factor) + np.nanmedian(an2 * correction_factor)
    return array

#calculate dF/F
def df(array):
    array = np.asarray(array)[CUT * SEC_HZ:]
    baseline_array = array[0: (BASE_LENGTH-CUT) * SEC_HZ]
    mean_baseline_array = np.nansum(baseline_array) / ((BASE_LENGTH-CUT) * SEC_HZ)
...
#low pass filter to remove motion artefacts
...
def linegraph(array, title):
    ...
    plt.xticks(np.arange(0, FRAME_LENGTH + 1, step=FRAME_LENGTH / 7), ['0', '5', '10', '15', '20', '25', '30', '35'], rotation='horizontal')
    plt.savefig(join(PATH, title + '.png'))
    # plt.show()
    plt.close()

#run main script for whole folder and save dF/F array as csv file for further processing
